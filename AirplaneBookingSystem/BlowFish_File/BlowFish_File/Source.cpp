...
static inline size_t input_string(uint8_t* buffer, fstream& text);
static inline void swap(uint32_t* N1, uint32_t* N2);
static inline void print_array(uint8_t* array, size_t length, int p, fstream& text);
static inline void print_bits(uint64_t x, register uint64_t Nbit);

// 18 * 32 = 576 bits
// PI = [K1..K18, S1..S4]
//ключи раунда
static uint32_t __Keys32b[18] = {
...
int main(void) {
    ...
    uint8_t enc_an_sim[BUFF_SIZE];
    int c;
    fstream keyFile("Key.txt");
    fstream text("Text.txt");
    fstream enc("Encrypted.txt");
    uint8_t key64b[56];
    if (keyFile.is_open())
    {
        keyFile >> key64b;
    ...
    size_t length = input_string(buffer, text);


    print_array(buffer, length, 0, enc);//
    ...
    print_array(encrypted, length, 1, enc);//
    encrypted[4] = 'r';
    length = blowfish(decrypted, 'D', __Keys32b, encrypted, length);
    fstream dec("Decrypted.txt");
    print_array(decrypted, length, 2, dec);
    //encrypted[4] = 'f';
    //encrypted[50] = ' ';
    //fstream enc_sim_w("en_w_sim.txt");
    //enc_sim_w << encrypted;
    //fstream dec_sim("dec_sim.txt");
    //length = blowfish(decrypted, 'D', __Keys32b, encrypted, length);
    //print_array(decrypted, length, 2, dec);
    //dec_sim << decrypted;
    return 0;
...
void key_extension(uint32_t* keys32b, uint8_t* keyNb, uint16_t len_bits) {
    const uint8_t block_size_in_bytes = len_bits / 8;//вычисляет размер в байтах
    for (uint8_t i = 0; i < 18; ++i) {
        keys32b[i] ^= join_8bits_to_32bits(
            keyNb + ((i * 4) % block_size_in_bytes)
        );
    }
    uint8_t init_blocks8b[8] = { 0 };//создаем инициализирующий блок 64 бита
...
size_t blowfish(uint8_t* to, uint8_t mode, uint32_t* keys32b, uint8_t* from, size_t length) {
    length = length % 8 == 0 ? length : length + (8 - (length % 8));//кратность блока = 64 бита
    uint32_t N1, N2;//блоки по 32 бита
...
static inline size_t input_string(uint8_t* buffer, fstream& text) {
    size_t position = 0;
    char ch;
    while ((text.read(&ch, sizeof ch)) && position < BUFF_SIZE - 1)
        buffer[position++] = ch;
    buffer[position] = '\0';
    text.close();
    return position;
