...
public class EducationWorkerRepository : IEducationWorker
{
    ...
    public async Task<ICollection<EducationWorker>>? GetEducationWorkersAsync() =>
        await _dbContext.EducationWorker.ToListAsync();


    public EducationWorker? GetEducationWorkerById(string IdEducationWorker) =>
        _dbContext.EducationWorker
        .FirstOrDefault(x => x.Id == IdEducationWorker);
    public async Task<EducationWorker>? GetEducationWorkerByIdAsync(string IdEducationWorker) =>
        await _dbContext.EducationWorker
        .FirstOrDefaultAsync(x => x.Id == IdEducationWorker);
    ...
    public ICollection<EducationWorker>? GetEducationWorkerByInstitution(string idInstitution) =>
        ...
        .ToList();
    public async Task<ICollection<EducationWorker>>? GetEducationWorkerByInstitutionAsync
